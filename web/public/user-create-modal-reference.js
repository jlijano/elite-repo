(function createUserReferenceModal() {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      body[data-admin-page="user"] #userDialog .reference-form-grid{display:grid!important;grid-template-columns:minmax(0,1fr) minmax(0,1fr)!important;gap:40px!important;align-items:start!important}
      body[data-admin-page="user"] #userDialog .reference-column{min-width:0!important;display:grid!important;gap:26px!important}
      ...
      body[data-admin-page="user"] #userDialog .reference-upload-content{display:grid!important;justify-items:center!important;gap:12px!important;text-align:center!important}
      ...
      body[data-admin-page="user"] #userDialog .photo-preview svg{width:54px!important;height:54px!important}
      ...
      body[data-admin-page="user"] #userDialog .reference-upload-title{color:#f8fafc!important;font-size:1.1rem!important;line-height:1.25!important}
      ...
      body[data-admin-page="user"] #userDialog .reference-status-field{position:relative!important}
      ...
      body[data-admin-page="user"] #userDialog .reference-helper{margin:-16px 0 0!important;color:rgba(203,213,225,.7)!important;font-size:.98rem!important;line-height:1.4!important}
      body[data-admin-page="user"] #userDialog .reference-password-shell{position:relative!important}
      ...
      body[data-admin-page="user"] #userDialog .form-actions button{min-width:128px!important;min-height:58px!important;padding:0 28px!important;border-radius:18px!important;font-size:1.02rem!important;font-weight:560!important;text-transform:none!important}
      ...
      @media (max-width:860px){body[data-admin-page="user"] #userDialog{width:calc(100vw - 24px)!important;max-height:calc(100dvh - 24px)!important}body[data-admin-page="user"] #userDialog .modal-head{min-height:74px!important;padding:0 22px!important}body[data-admin-page="user"] #userDialog .user-dialog-body{padding:24px!important}body[data-admin-page="user"] #userDialog .reference-form-grid{grid-template-columns:1fr!important;gap:24px!important}}
      @media (max-width:520px){body[data-admin-page="user"] #userDialog .user-dialog-body{padding:18px!important}body[data-admin-page="user"] #userDialog .photo-drop-zone{min-height:220px!important}body[data-admin-page="user"] #userDialog .form-actions{flex-direction:column-reverse!important}body[data-admin-page="user"] #userDialog .form-actions button{width:100%!important}}
    `;
  ...
  function moveField(column, field) {
  ...
  function enhanceUploadBox() {
    ...
    if (chooseButton) chooseButton.textContent = "Choose File";
  ...
  function reshapeForm() {
    ...
    moveField(left, document.querySelector(".photo-field"));
    topRow?.remove();
    moveField(left, fieldFor("userName"));
    moveField(left, fieldFor("userEmail"));
    moveField(left, fieldFor("userPhotoUrl"));
    const statusField = fieldFor("userStatus");
    if (statusField) {
      statusField.classList.add("reference-status-field");
      moveField(left, statusField);
    ...
    moveField(right, fieldFor("userCompany"));
    moveField(right, fieldFor("userTitle"));
    moveField(right, fieldFor("userGroup"));
    const passwordField = fieldFor("userPassword");
    if (passwordField) {
      ...
      moveField(right, passwordField);
    }
    moveField(right, fieldFor("userRole"));
    form.querySelector(".user-role-status-row")?.remove();
