...
(() => {
  ...
  function isLoggedIn() {
    return typeof currentSessionToken === "function" ? Boolean(currentSessionToken()) : Boolean(sessionGet("switchboard-session-token"));
  }
  ...
  function installGuestGate() {
    if (typeof createChat === "function" && createChat.__guestAccessEnhanced !== true) {
    ...
    if (typeof sendMessage === "function" && sendMessage.__guestAccessEnhanced !== true) {
  ...
  new MutationObserver(addExtraColorOptions).observe(document.body, { childList: true, subtree: true });
  setTimeout(() => {
